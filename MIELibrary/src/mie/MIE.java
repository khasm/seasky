...
public interface MIE {
	...
	/**
	 * Adds a mime document to the repository. The document will be parsed and text and jpeg images will be processed
	 * with cbir. 
	 ...
	 * @param name name of the document
	 * @return the document contents or null if the document didn't exist or there was a problem connecting
	 * to the server
	 */
	public byte[] getMime(String name, boolean useCache);
	
	/**
	 * Search the repository for documents that match the text and image components of the provided document
	...
	/**
	 * Adds an unstructured document composed of an image and a text component to the server. Unlike mime, each document 
	 * can only have at most one of each components
	 * @param name name of the document
	...
	/**
	 * Retrieves the unstructured document with the given id from the server. An unstructured document is composed of
	 * an image, text, or both. If either the image or the text component doesn't exist an array of length 0 is returned
	 * on its place.
	 * @param name the name of the document
	 * @return an array of arrays of bytes of length 2. the first position of the array of arrays will contain the image
	 * contents, or an array of length 0 if the document doesn't have an image, while the second position will contain the
	 * text contents, or an array of length 0 if there is no text component. Returns null if there was an error
	 * connecting to the server
	 */
	public byte[][] getUnstructuredDoc(String name, boolean useCache);
	
	/**
	 * Searchs the server for documents that match the features of the given image and/or text components. An ordered list
	 * will be return with both the id and the score of the matching documents
	 * @param img the image contents
	 * @param txt the text contents
	 * @param nResults maximum number of results returned
	 * @return an ordered list with the matching documents, or null if there was an error connecting to the server
	 */
	public List<SearchResult> searchUnstructuredDocument(byte[] img, byte[] txt, int nResults);
	
	/**
	 * Sends the index command to the server. This will start the training and indexing operation in the background
