...
double readDoubleFromArr (const char* arr, int* pos) {
    uint64_t x;
    readFromArr(&x, sizeof(uint64_t), arr, pos);
...
std::set<QueryResult,cmp_QueryResult> sort (std::map<string,double>* queryResults) {
    std::set<QueryResult,cmp_QueryResult> orderedResults;
    for (std::map<string,double>::iterator it=queryResults->begin(); it!=queryResults->end(); ++it) {
...
std::set<QueryResult,cmp_QueryResult> mergeSearchResults(std::set<QueryResult,cmp_QueryResult>* imgResults,
    ...
    if (imgResults != NULL) {
        ...
        for (std::set<QueryResult,cmp_QueryResult>::iterator it=imgResults->begin(); it!=imgResults->end(); ++it) {
            std::map<string,Rank>::iterator rank = ranks.find(it->docId);
            if (rank == ranks.end()) {
                struct Rank qr;
                qr.textRank = 0;
                qr.imgRank = i++;
    ...
    std::map<string,double> queryResults;
    for (std::map<string,Rank>::iterator it=ranks.begin(); it!=ranks.end(); ++it) {
...
void sendQueryResponse(int newsockfd, std::set<QueryResult,cmp_QueryResult>* mergedResults, size_t resultsSize) {
    ...
    addIntToArr (size-2*sizeof(int), buff, &pos);
    size_t i = 1;
    for (std::set<QueryResult,cmp_QueryResult>::iterator it = mergedResults->begin(); it != mergedResults->end(); ++it) {
        double score = it->score;
        addIntToArr(it->docId.size(), buff, &pos);
        addToArr((void*)it->docId.c_str(), it->docId.size(), buff, &pos);
